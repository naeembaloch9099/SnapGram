...
const StoryViewer = ({
  ...
  const duration = current?.type === "video" ? 15000 : 7000; // 15s video, 7s image
  ...
  const probeMedia = async (url, timeoutMs = 8000) => {
    if (!url) return;
    setDebugInfo({ status: "probing", url, timestamp: Date.now() });
    ...
    try {
      ...
      console.debug("StoryViewer: probe result", info);
      setDebugInfo(info);
      return info;
    } catch (err) {
      clearTimeout(id);
      ...
      console.debug("StoryViewer: probe failed", info);
      setDebugInfo(info);
      return info;
    }
  };
  ...
  const fetchMediaViaProxy = async (storyId) => {
    ...
    try {
      ...
      console.warn("fetchMediaViaProxy failed", err?.message || err);
      return null;
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-12 left-4 right-4 flex justify-between items-center z-10">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => probeMedia(current.url)}
            className="text-white text-sm bg-white/10 px-3 py-1 rounded"
            title="Run media probe"
          >
            Probe
          </button>
        ...
        {current.type === "video" ? (
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            <video
              ...
              onError={() => {
                console.debug(
                  "StoryViewer: video failed to load:",
                  current?.url
                );
                setMediaError(true);
                probeMedia(current?.url);
          ...
          <img
            ...
            className="max-h-full max-w-full object-contain"
            onError={(e) => {
              console.debug("StoryViewer: image failed to load:", current?.url);
      ...
      {/* Debug panel */}
      {debugInfo && (
        <div className="absolute bottom-4 right-4 z-30 bg-black/70 text-white text-xs p-2 rounded max-w-sm">
          <div className="font-medium">Media Probe</div>
          <div className="truncate">URL: {debugInfo.url}</div>
          {debugInfo.status && <div>Status: {debugInfo.status}</div>}
          {typeof debugInfo.ok !== "undefined" && (
            <div>OK: {String(debugInfo.ok)}</div>
          )}
          {debugInfo.status && (
            <div>
              HTTP: {debugInfo.status} {debugInfo.statusText}
            </div>
          )}
          {debugInfo.contentType && <div>Type: {debugInfo.contentType}</div>}
          ...
          <div className="mt-2 flex gap-2">
            <button
              ...
              onClick={async () => {
                try {
                  await navigator.clipboard.writeText(debugInfo.url || "");
                } catch (e) {
